...
function CustomSelect({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div ref={ref} className="relative">
      <label htmlFor="subject-select" className="sr-only">{t.contact.form.subjectLabel}</label>
      <button
        id="subject-select"
        type="button"
        role="combobox"
  ...
  if (submitted) {
    return (
      <div className="border-3 border-accent p-10 text-center" role="status" aria-live="polite">
        ...
        <p className="font-mono text-sm text-muted-light dark:text-muted-dark">{t.contact.success.message}</p>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true) }} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label htmlFor="contact-name" className="sr-only">{f.nameLabel}</label>
        ...
        <div>
          <label htmlFor="contact-email" className="sr-only">{f.emailLabel}</label>
      ...
      <div>
        <label htmlFor="contact-company" className="sr-only">{f.companyLabel}</label>
      ...
      <div>
        <label htmlFor="contact-message" className="sr-only">{f.messageLabel}</label>
      ...
      <button type="submit" className="w-full md:w-auto bg-accent text-white border-3 border-accent px-8 py-3 font-mono text-sm font-extrabold uppercase tracking-widest glow-accent transition-[background-color,color] duration-200 hover:bg-transparent hover:text-accent cursor-pointer touch-manipulation">{f.submit}</button>
    </form>
  )
}
