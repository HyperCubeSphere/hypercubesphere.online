    ...
    <div>
      <section className="border-b-3 border-border-light dark:border-border-dark">
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 py-16 md:py-24">
          ...
          <p className="text-sm text-muted-light dark:text-muted-dark max-w-xl leading-relaxed">{s.subtitle}</p>
        </div>
      </section>
              ...
              <div key={service.number} className="border-3 border-border-light dark:border-border-dark p-8 md:p-10 -mt-[3px] first:mt-0 group transition-[border-color,background-color] duration-200 relative z-0 hover:z-10 hover:border-accent hover:bg-hover-light dark:hover:bg-hover-dark">
                <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.5fr] gap-8">
                  <div>
                    ...
                    <p className="text-[13px] text-muted-light dark:text-muted-dark leading-relaxed">{service.description}</p>
                  </div>
                  <div>
                    ...
                    <ul className="grid grid-cols-1 md:grid-cols-2 gap-2">
                      {service.capabilities.map((cap) => (
                        <li key={cap} className="text-[13px] text-muted-light dark:text-muted-dark flex items-start gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0">
            {s.differentiators.map((d, i) => (
              <div key={d.label} className="border-3 border-border-light dark:border-border-dark p-6 -m-[1.5px] brutalist-hover">
                ...
                <p className="text-[13px] text-muted-light dark:text-muted-dark leading-relaxed">{d.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="border-t-3 border-accent">
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 py-24 md:py-32">
          <h2 className="text-3xl md:text-5xl font-extrabold leading-[1.05] tracking-tight mb-8 max-w-2xl">{s.ctaTitle}</h2>
