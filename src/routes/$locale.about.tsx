    ...
    <div>
      <section className="border-b-3 border-border-light dark:border-border-dark">
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 py-16 md:py-24">
          ...
          <p className="text-sm text-muted-light dark:text-muted-dark max-w-xl leading-relaxed">{a.subtitle}</p>
        </div>
      </section>
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 py-16">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <div>
              ...
              <div className="space-y-4 text-[13px] text-muted-light dark:text-muted-dark leading-relaxed">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0">
            {a.values.map((v) => (
              <div key={v.prefix} className="border-3 border-border-light dark:border-border-dark p-6 -m-[1.5px] brutalist-hover">
                ...
                <p className="text-[13px] text-muted-light dark:text-muted-dark leading-relaxed">{v.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section>
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 py-8">
          ...
          <h2 className="text-2xl md:text-3xl font-extrabold uppercase tracking-wider">{a.metricsTitle}</h2>
        </div>
        <div className="border-t-3 border-b-3 border-border-light dark:border-border-dark">
          <div className="max-w-[1400px] mx-auto grid grid-cols-2 lg:grid-cols-4">
            {a.metrics.map((m, i) => (
              <StatBlock key={i} value={m.value} label={m.label} />
            ))}
          </div>
        </div>
      </section>
